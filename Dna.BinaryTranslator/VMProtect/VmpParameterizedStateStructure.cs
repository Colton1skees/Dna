...
namespace Dna.BinaryTranslator.VMProtect
{
    ...
    /// </summary>
    public class VmpParameterizedStateStructure
    {
        ...
        public static LLVMTypeRef CreateParameterizedPrototype(LLVMContextRef ctx, LLVMTypeRef stateStructType, IReadOnlyDictionary<RemillRegister, int> registerArgumentIndices, bool addMemoryPtr)
        {
            ...
            var prototype = LLVMTypeRef.CreateFunction(LLVMTypeRef.Void, paramTypes.ToArray());
            return prototype;
        }
        ...
        private void Create(LLVMValueRef inputFunction)
        {
            ...
            {
                var reg = OrderedRegisterArguments[i];
                var regPtr = reg.GetAddressOf(localStateStruct, builder);
                var regValue = builder.BuildLoad2(LLVMTypeRef.Int64, regPtr);
                builder.BuildStore(regValue, newFunc.GetParam((uint)i + baseOffset));
            }
        ...
        private LLVMValueRef CreateCallToOriginalFunction(LLVMValueRef originalFunc, LLVMValueRef newFunc, LLVMValueRef localStateStruct)
        {
            ...
            callArgs.Add(GetRegParam(rootRegisters.Single(x => x.Name == "RDX"), newFunc));
