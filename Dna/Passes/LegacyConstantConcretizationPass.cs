...
{
    public class LegacyConstantConcretizationPass
    {
        ...
        public LegacyConstantConcretizationPass(LLVMValueRef function, LLVMBuilderRef builder, IBinary binary)
        {
        ...
        private void TrackSecionAccesses(LLVMValueRef value, uint bitWidth)
        {
            ...
            if (!BinaryAccessMatcher.IsBinarySectionAccess(binary, value.GetOperand(1)))
                return;

            // Get the binary section offset.
            var sectionOffset = BinaryAccessMatcher.GetBinarySectionOffset(binary, value.GetOperand(1));
