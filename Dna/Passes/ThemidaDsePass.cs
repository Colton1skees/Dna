...
{
    // Legacy pass originally used to perform dead store elimination and redundant load elimination over themida protected functions.
    // At one point this file also contained a control flow structuring pass....
    // TODO: Delete me
    public class ThemidaDsePass
    {
        ...
        private LLVMValueRef? TryResolveLoad(LLVMValueRef loadInst, MemorySSA mssa, IBinary binary)
        {
            ...
            if (!BinaryAccessMatcher.IsConstantWithinBinarySection(binary,gepIndex))
                return null;
            ...
            foreach (var clobberInst in clobberingAccesses.Reverse().Select(x => x.MemoryInst))
            {
                ...
                if (!BinaryAccessMatcher.IsConstantWithinBinarySection(binary, storeGepIndex))
                    return null;
                ...
                var storeOffset = BinaryAccessMatcher.GetBinarySectionOffset(binary, storeGepIndex);
            ...
            var loadOffset = BinaryAccessMatcher.GetBinarySectionOffset(binary, gepIndex);
